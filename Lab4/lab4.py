...
def rsa_encrypt_file(plaintext: bytes, pub_key: RSAPublicKey) -> bytes:
...
def rsa_decrypt_file(data: bytes, priv_key: RSAPrivateKey) -> bytes:
    out = bytearray()
    i = 0
    while i < len(data):
...
def run_ui() -> None:
    ...
    frm_ks.pack()
    tk.Label(frm_ks, text="Key size (bits):").pack(side="left")
    key_size_var = tk.StringVar(value="2048")
    tk.OptionMenu(frm_ks, key_size_var, "1024", "2048", "4096").pack(side="left", padx=4)
    ...
    def do_generate() -> None:
        ...
        try:
            ...
            kset(f"Generating {bits}-bit key…"); root.update()
            priv_key, pub_key = generate_keys(bits)
            kset(f"{bits}-bit key pair generated (not saved)")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def do_save_keys() -> None:
        if not priv_key:
            messagebox.showerror("Error", "Generate keys first."); return
        ...
        kset(f"Saved: {os.path.basename(priv)}, {os.path.basename(pub)}")
    ...
    def do_load_private() -> None:
        ...
        try:
            ...
            kset(f"Loaded private key: {os.path.basename(path)}")
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def do_load_public() -> None:
        ...
        try:
            ...
            kset(f"Loaded public key: {os.path.basename(path)}")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    frm_k = tk.Frame(root); frm_k.pack(pady=3)
    ...
    tk.Button(frm_k, text="Load Private Key", command=do_load_private).pack(side="left", padx=3)
    tk.Button(frm_k, text="Load Public Key",  command=do_load_public).pack(side="left", padx=3)
    tk.Label(root, text="File Encryption").pack(pady=(10, 0))
    ...
    def do_encrypt() -> None:
        if not pub_key:
            messagebox.showerror("Error", "Load or generate a public key first."); return
        src = filedialog.askopenfilename(title="File to encrypt")
        if not src: return
        dst = filedialog.asksaveasfilename(
            title="Save encrypted file",
            defaultextension=".rsa",
            filetypes=[("RSA file", "*.rsa"), ("All", "*.*")]
        )
        if not dst: return
        try:
            with open(src, 'rb') as f: data = f.read()
            oset("Encrypting…"); root.update()
            t0 = time.perf_counter()
            enc = rsa_encrypt_file(data, pub_key)
            elapsed = time.perf_counter() - t0
            with open(dst, 'wb') as f: f.write(enc)
            oset(f"Encrypted {len(data)/1024:.1f} KB in {elapsed:.3f}s → {os.path.basename(dst)}")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def do_decrypt() -> None:
        if not priv_key:
            messagebox.showerror("Error", "Load or generate a private key first."); return
        ...
        try:
            with open(src, 'rb') as f: data = f.read()
            oset("Decrypting…"); root.update()
            t0 = time.perf_counter()
            plain = rsa_decrypt_file(data, priv_key)
            elapsed = time.perf_counter() - t0
            with open(dst, 'wb') as f: f.write(plain)
            oset(f"Decrypted {len(plain)/1024:.1f} KB in {elapsed:.3f}s → {os.path.basename(dst)}")
        except Exception as e:
            messagebox.showerror("Error", str(e))

    frm_f = tk.Frame(root); frm_f.pack(pady=3)
    tk.Button(frm_f, text="Encrypt File", command=do_encrypt).pack(side="left", padx=3)
    tk.Button(frm_f, text="Decrypt File", command=do_decrypt).pack(side="left", padx=3)
    root.mainloop()
